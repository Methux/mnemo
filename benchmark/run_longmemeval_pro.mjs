...
async function main() {
  ...
  console.log(`\n${"=".repeat(60)}`);
  console.log(`LongMemEval Benchmark — mnemo-pro (batch embed)`);
  ...
  const db = await lancedb.connect("/tmp/mnemo-benchmark-pro");
  ...
  mnemo = await createMnemo({
    dbPath: "/tmp/mnemo-benchmark-pro",
    embedding: {
      apiKey: VOYAGE_KEY,
      baseURL: "https://api.voyageai.com/v1",
      model: "voyage-4",
  ...
  const outFile = join(RESULTS_DIR, `longmemeval_mnemo-pro_${new Date().toISOString().slice(0, 10)}.json`);
