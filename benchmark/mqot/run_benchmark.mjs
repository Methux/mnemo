...
async function judgeAnswer(question, predicted, gold, category, oldAnswer) {
  ...
  // Special scoring for "contradiction" category
  if (category === "contradiction" && oldAnswer) {
    const lower = predicted.toLowerCase();
    const oldLower = oldAnswer.toLowerCase();
    // If answer matches OLD value, score 0 (used stale data)
    if (lower.includes(oldLower) && !lower.includes(gold.toLowerCase())) return 0;
  }
...
async function main() {
  ...
  console.log(`  Important: ${dataset.important_facts.length}, Trivial: ${dataset.trivial_facts.length}, Updates: ${dataset.fact_updates.length}`);
  ...
  // Recall important facts multiple times (simulates real usage)
  ...
  for (let cycle = 0; cycle < 3; cycle++) {
    ...
    console.log(`  Recall cycle ${cycle + 1}/3 complete — ${((Date.now() - t1) / 1000).toFixed(0)}s`);
  }
